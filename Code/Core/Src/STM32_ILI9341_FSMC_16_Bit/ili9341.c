...
void lcdDrawImage(uint16_t x, uint16_t y, GUI_CONST_STORAGE GUI_BITMAP* pBitmap)
{
	...
	uint32_t dataLength = pBitmap->bytesPerLine / (pBitmap->bitsPerPixel / 8);

	for (int i = 0; i < pBitmap->ySize; i++)
		lcdDrawPixels(x, y + i, (uint16_t*)(pBitmap->pData + i * pBitmap->bytesPerLine), dataLength);
}
...
static uint16_t TSC2046_SendCommand(uint8_t cmd) {
	...
	lcdBuf[0] = cmd;
	HAL_SPI_Transmit(_hspi, lcdBuf, 1, 10);
	if (HAL_SPI_Receive(_hspi, &lcdBuf[1], 2, 10) == HAL_OK)
...
static uint16_t TSC2046_getRaw_X() {
	return TSC2046_SendCommand(CMD_X_AXIS | 0x84); // 0x84 = Command default
}
// Get y-axis raw touch 12-bit value
static uint16_t TSC2046_getRaw_Y() {
	return TSC2046_SendCommand(CMD_Y_AXIS | 0x84); // 0x84 = Command default
}
/*Get z-axis raw touch 12-bit value - Not used any longer due to TS_IRQ
static uint16_t TSC2046_getRaw_Z() {
	return TSC2046_SendCommand(CMD_Z_AXIS | 0x84); // 0x84 = Command default
}*/

// Print calibration point - Top-Left corner point
static void TSC2046_TL_point() {
	lcdFillCircle(10, 10, 3, COLOR_RED);
	lcdSetCursor(20, 30);
	lcdSetTextColor(COLOR_RED, COLOR_BLACK);
	lcdPrintf("Press here");
}
// Print calibration point - Bottom-Right corner point
static void TSC2046_BR_point() {
	lcdFillCircle(lcdProperties.width - 10, lcdProperties.height - 10, 3, COLOR_RED);
	lcdSetCursor(lcdProperties.width - 80, lcdProperties.height - 40);
	lcdSetTextColor(COLOR_RED, COLOR_BLACK);
	lcdPrintf("Press here");
}
...
static void TSC2046_GetRawTouch() {
	...
	switch (lcdProperties.orientation) {
	case LCD_ORIENTATION_LANDSCAPE_MIRROR:
		lcdProperties.localRawTouch.x_touch = 4095 - TSC2046_getRaw_X();
		lcdProperties.localRawTouch.y_touch = TSC2046_getRaw_Y();
		lcdProperties.myTS_Calibrate.Width = 240;
		lcdProperties.myTS_Calibrate.Height = 320;
		break;

	case LCD_ORIENTATION_LANDSCAPE:
		lcdProperties.localRawTouch.x_touch = 4095 - TSC2046_getRaw_Y();
		lcdProperties.localRawTouch.y_touch = 4095 - TSC2046_getRaw_X();
		lcdProperties.myTS_Calibrate.Width = 320;
		lcdProperties.myTS_Calibrate.Height = 240;
		break;

	case LCD_ORIENTATION_PORTRAIT:
		lcdProperties.localRawTouch.x_touch = TSC2046_getRaw_X();
		lcdProperties.localRawTouch.y_touch = 4095 - TSC2046_getRaw_Y();
		lcdProperties.myTS_Calibrate.Width = 240;
		lcdProperties.myTS_Calibrate.Height = 320;
		break;

	case LCD_ORIENTATION_PORTRAIT_MIRROR:
		lcdProperties.localRawTouch.x_touch = TSC2046_getRaw_Y();
		lcdProperties.localRawTouch.y_touch = TSC2046_getRaw_X();
		lcdProperties.myTS_Calibrate.Width = 320;
		lcdProperties.myTS_Calibrate.Height = 240;
		break;
	}
	// These are for the calibration
	lcdProperties.myRawTouchDef.x_touch = lcdProperties.localRawTouch.x_touch;
	lcdProperties.myRawTouchDef.y_touch = lcdProperties.localRawTouch.y_touch;
}

// Get touch screen data when calibration is already made
static void TSC2046_GetData() {
	// Reset
	uint16_t temp16x = 0, temp16y = 0;
	// Read touch data
	for (uint8_t i = 0; i < 10; i++) {
		TSC2046_GetRawTouch();
		temp16x += lcdProperties.localRawTouch.x_touch;
		temp16y += lcdProperties.localRawTouch.y_touch;
	}
	lcdProperties.localRawTouch.x_touch = temp16x * 0.1;
	lcdProperties.localRawTouch.y_touch = temp16y * 0.1;

	// X-axis value
	lcdProperties.myTsData.X = lcdProperties.myTS_Calibrate.Scale_X * lcdProperties.localRawTouch.x_touch + lcdProperties.myTS_Calibrate.Bias_X;

	// Y-axis value
	lcdProperties.myTsData.Y = lcdProperties.myTS_Calibrate.Scale_Y * lcdProperties.localRawTouch.y_touch + lcdProperties.myTS_Calibrate.Bias_Y;
}
...
void touchCalibrate() {
	...
	// Reset
	lcdProperties.myTS_Calibrate.TL_X = 0;
	lcdProperties.myTS_Calibrate.TL_Y = 0;
	lcdProperties.myTS_Calibrate.BR_X = 0;
	lcdProperties.myTS_Calibrate.BR_Y = 0;
	lcdProperties.myRawTouchDef.y_touch = 0;
	lcdProperties.myRawTouchDef.x_touch = 0;
	...
	while (1) {
		if (touchIsPressed()) {
			for (uint8_t i = 0; i < 10; i++) {
				TSC2046_GetRawTouch();
				lcdProperties.myTS_Calibrate.TL_X += lcdProperties.myRawTouchDef.x_touch;
				lcdProperties.myTS_Calibrate.TL_Y += lcdProperties.myRawTouchDef.y_touch;
			}
			break;
		}
		HAL_Delay(10);
	...
	while (1) {
		if (touchIsPressed()) {
			for (uint8_t i = 0; i < 10; i++) {
				TSC2046_GetRawTouch();
				lcdProperties.myTS_Calibrate.BR_X += lcdProperties.myRawTouchDef.x_touch;
				lcdProperties.myTS_Calibrate.BR_Y += lcdProperties.myRawTouchDef.y_touch;
			}
			break;
		}
		HAL_Delay(10);
	...
	lcdProperties.myTS_Calibrate.TL_X *= 0.1;
	lcdProperties.myTS_Calibrate.TL_Y *= 0.1;

	lcdProperties.myTS_Calibrate.BR_X *= 0.1;
	lcdProperties.myTS_Calibrate.BR_Y *= 0.1;

	// Calculate scalar
	lcdProperties.myTS_Calibrate.Scale_X = (lcdProperties.myTS_Calibrate.Width + 0.0f) / (lcdProperties.myTS_Calibrate.BR_X - lcdProperties.myTS_Calibrate.TL_X + 0.0f);
	lcdProperties.myTS_Calibrate.Scale_Y = (lcdProperties.myTS_Calibrate.Height + 0.0f) / (lcdProperties.myTS_Calibrate.BR_Y - lcdProperties.myTS_Calibrate.TL_Y + 0.0f);

	// Calculate bias
	lcdProperties.myTS_Calibrate.Bias_X = -1 - lcdProperties.myTS_Calibrate.Scale_X * lcdProperties.myTS_Calibrate.TL_X; // -1 is for bias. Change this so you have 100% accuracy at center of the LCD
	lcdProperties.myTS_Calibrate.Bias_Y = -1 - lcdProperties.myTS_Calibrate.Scale_Y * lcdProperties.myTS_Calibrate.TL_Y; // -1 is for bias. Change this so you have 100% accuracy at center of the LCD
}

void touchGetCalibrationParameters(float *Scale_X, float *Scale_Y, float *Bias_X, float *Bias_Y) {
	*Scale_X = lcdProperties.myTS_Calibrate.Scale_X;
	*Scale_Y = lcdProperties.myTS_Calibrate.Scale_Y;
	*Bias_X = lcdProperties.myTS_Calibrate.Bias_X;
	*Bias_Y = lcdProperties.myTS_Calibrate.Bias_Y;
}

void touchSetCalibrationParameters(float *Scale_X, float *Scale_Y, float *Bias_X, float *Bias_Y) {
	lcdProperties.myTS_Calibrate.Scale_X = *Scale_X;
	lcdProperties.myTS_Calibrate.Scale_Y = *Scale_Y;
	lcdProperties.myTS_Calibrate.Bias_X = *Bias_X;
	lcdProperties.myTS_Calibrate.Bias_Y = *Bias_Y;
}

bool touchIsPressed() {
	static uint8_t count = 0;
	static bool has_been_pressed = false;
	bool pressed = (bool) !HAL_GPIO_ReadPin(TOUCH_IRQ_GPIO_Port, TOUCH_IRQ_Pin); // TS_IRQ goes low when LCD being pressed on
...
void touchPrintCoordinates(){
	TSC2046_GetData();
	char text[30];
	sprintf(text, "X:%i,Y:%i", lcdProperties.myTsData.X, lcdProperties.myTsData.Y);
	lcdFillRGB(COLOR_BLACK);
	lcdSetTextColor(COLOR_RED, COLOR_BLACK);
	lcdSetCursor(lcdProperties.myTsData.X, lcdProperties.myTsData.Y);
	lcdPrintf(text);
}
